...
#### check this function in real life 
rename_counties <- function(x){
  x %>%
...
acs5_median_age_usa <- get_acs(
  geography = "us",
  variables = c("median_age_total" = "B01002_001"),
  cache_table = TRUE,
  year = 2018,
  survey = "acs5"
)
...
acs5_median_age_catchment <- get_acs(
  geography = "county",
  variables = c(
...
median_age <- get_acs(
  geography = "county",
  state = "az",
  variables = c("median_age_white" = "B01002A_001",
...
median_age %>%
  ...
  spread(key = variable,
         value = estimate) %>%
  summarise(min(median_age_white),
            max(median_age_white))
...
median_age %>%
  ...
  spread(key = variable,
         value = estimate) %>%
  summarise(min(median_age_black),
            max(median_age_black))
...
age_65_catchment_race_white <- get_acs(
  geography = "county",
  state = "az",
  variables = c(
    "white_total" = "B01001H_001",
...
age_65_catchment_race_white_total <- age_65_catchment_race_white %>%
  mutate(NAME = str_replace(age_65_catchment_race_white$NAME, " County, Arizona", "")) %>%
  filter(NAME %in% counties,
         variable == "white_total") %>%
  summarize(white_total = (sum(estimate)))

age_65_catchment_race_white %>%
  mutate(NAME = str_replace(age_65_catchment_race_white$NAME, " County, Arizona", "")) %>%
  filter(NAME %in% counties,
         variable != "white_total") %>%
